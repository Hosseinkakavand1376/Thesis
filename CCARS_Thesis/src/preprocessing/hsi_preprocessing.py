...
Apply Log10 + SNV normalization following Nicola's exact methodology
"""
...
def preprocess_hsi_data(X_df, apply_log=True, apply_snv=True, epsilon=1e-10):
    """
    Complete preprocessing pipeline: Log10 → SNV
    
    CRITICAL: Order matters! Log10 must be applied before SNV
    This exactly replicates Nicola's preprocessing
    
    Args:
        X_df: Input DataFrame with reflectance values
        apply_log: If True, apply log10 transformation
        apply_snv: If True, apply SNV normalization
    ...
    print(f"Pipeline: {'Log10' if apply_log else 'No Log'} → {'SNV' if apply_snv else 'No SNV'}")
    print()
    ...
    if apply_log:
        X_processed = apply_log10_transform(X_processed, epsilon=epsilon)
...
def preprocess_train_test(X_train_df, X_test_df, apply_log=True, apply_snv=True):
    """
    ...
    Args:
        ...
        apply_log: Apply log10 transformation
        apply_snv: Apply SNV normalization
    ...
    print("\n📊 Training set:")
    X_train_processed = preprocess_hsi_data(X_train_df, apply_log, apply_snv)
    
    print("\n📊 Test set:")
    X_test_processed = preprocess_hsi_data(X_test_df, apply_log, apply_snv)
...
if __name__ == '__main__':
    ...
    # Test preprocessing
    X_processed = preprocess_hsi_data(X_df_synthetic, apply_log=True, apply_snv=True)
    
    # Validate
    validate_preprocessing(X_df_synthetic, X_processed, n_samples=3)
