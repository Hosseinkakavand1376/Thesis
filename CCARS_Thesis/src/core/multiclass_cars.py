...
class MultiClassCARS:
    ...
    def __init__(self, output_path, n_components=3, cv_fold=5,
                 test_percentage=0.2, calibration=True, random_state=42):
        """
        ...
        Args:
            ...
            test_percentage: Fraction for test split
            calibration: Whether to use calibration split
    ...
    def fit(self, X_train, y_train, X_test, y_test, wavelengths):
        ...
        Args:
            X_train: (n_train, n_features) training spectral data
            y_train: (n_train,) training labels (0-indexed)
            X_test: (n_test, n_features) test spectral data
            y_test: (n_test,) test labels (0-indexed)
    ...
    def _compute_cars(self, K, N, start_run, end_run, use_ars):
        ...
        for run_idx in tqdm(range(start_run, end_run), desc="CARS Runs"):
            ...
            for iter_idx in range(1, N + 1):
                ...
                coef_mean = pls_model.coef_.mean(axis=1)  # Mean across classes
