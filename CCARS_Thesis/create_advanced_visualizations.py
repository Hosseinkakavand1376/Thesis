...
def create_feature_importance_plot(dataset_name, output_dir):
    ...
    # Try to find wavelength file in new multi-component structure or old flat structure
    base_path = Path(f'HSI_CARS_comprehensive/{dataset_name}')
    wl_path = None
    
    # Check new multi-component structure first (components_N/wavelength_*)
    for component_dir in sorted(base_path.glob('components_*')):
        for subdir in sorted(component_dir.glob('wavelength_*')):
    ...
    # Fallback: check old flat structure (wavelength_*)
    if wl_path is None:
